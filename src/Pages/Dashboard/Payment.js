...
const Payment = () => {
  ...
  const { data: booking, isLoading } = useQuery(['bookings', id], () =>
    fetch(url, {
      method: 'GET',
      headers: {
      ...
      <div className="card w-50 max-w-md bg-teal-50 shadow-xl">
        <div className="card-body">
          ...
          <h2 className="text-lg">
            You have ordered for {booking.orderQuantity} {booking.product}.
          </h2>
          <h2>
            Total cost of your order is: <span className="text-secondary font-semibold">${booking.cost}</span>{' '}
          </h2>
        </div>
      </div>
